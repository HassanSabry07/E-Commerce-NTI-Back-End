...
const populateCart = (query) =>
  query.populate("items.product", "title price image stock slug category");
...
exports.addToCart = async (req, res) => {
  try {
    ...
    if (!cart) {
      cart = new Cart({
        user: userId,
        items: [{ product: productId, quantity, priceAtAdding: product.price }],
      });
    } else {
      const item = cart.items.find((i) => i.product.toString() === productId);
      if (item) {
        item.quantity += quantity;
        item.priceAtAdding = product.price;
      } else
        cart.items.push({
          product: productId,
          quantity,
