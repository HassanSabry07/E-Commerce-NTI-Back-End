...
exports.getWishlist = async (req, res) => {
  try {
    const wishlist = await Wishlist.findOne({ user: req.user._id }).populate(
      "items.product",
      "title price image stock slug category",
    );
  ...
  try {
    ...
    if (!wishlist) {
      wishlist = new Wishlist({
        user: userId,
        items: [{ product: productId, priceAtAdding: product.price }],
      });
    } else {
  ...
  try {
    ...
    if (!cart) {
      cart = new Cart({
        user: userId,
        items: [
          { product: productId, quantity: 1, priceAtAdding: product.price },
        ],
      });
    } else {
      ...
      if (cartItem) {
        cartItem.quantity += 1;
        cartItem.priceAtAdding = product.price;
      } else
        cart.items.push({
          product: productId,
          quantity: 1,
